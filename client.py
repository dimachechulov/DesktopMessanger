...
def display_previous_message(sock, account_name, receiver_name):
    message_dict={
        ACTION: PREVIOUS,
        SENDER: account_name,
        DESTINATION: receiver_name,
        ...
        try:
            ...
            #предыдущие сообщения
                ...
                for msg in message['messages']:
                    if user_name==msg[SENDER]:
                        print(f'[you] {msg[MESSAGE_TEXT]}')
                    else:
...
def user_controls_cmd(sock, user_name):
    ...
    while True:
            ...
            receiver_name=input('Введите получателя сообщения(или /exit чтобы выйти): ')
            user_status = receiver_name
            display_previous_message(sock, user_name, receiver_name)
            if receiver_name == "/exit":
                break
            while True:
                if create_client_msg(sock, user_name, receiver_name) == 'end':
                    break
            user_status=None
